...
function c24562465.e4cfil2(c,tp)
	return c:IsSetCard(0x9390) and c:IsDiscardable()
end
...
function c24562465.e4op(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		if c:IsFaceup() and Duel.GetMatchingGroupCount(c24562465.e4mtfil,tp,LOCATION_REMOVED,0,nil)>0 then
			Duel.BreakEffect()
...
function c24562465.e3op(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,24562464,0x9390,0x4011,700,700,1,RACE_ROCK,ATTRIBUTE_EARTH) then return end
...
function c24562465.e3tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,59822133)
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,24562464,0x9390,0x4011,700,700,1,RACE_ROCK,ATTRIBUTE_EARTH) end
