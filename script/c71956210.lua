...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.ShuffleDeck(tp)
	local ct1=Duel.GetOperatedGroup():FilterCount(cm.locfilter,nil,tp)
	local ct2=Duel.GetOperatedGroup():FilterCount(cm.locfilter,nil,1-tp)
	...
	ct1=ct1-1
	while ct1>0 and g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(m,0)) do
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	...
	if ct2>0 and Duel.IsExistingMatchingCard(Card.IsCanBeSpecialSummoned,tp,0,LOCATION_DECK,1,nil,e,0,1-tp,false,false)
		and Duel.SelectYesNo(1-tp,aux.Stringid(cm,1)) then
		Duel.BreakEffect()
