...
function cm.filter(c)
	return c:IsFaceup() and c:IsSetCard(0xde5) and c:IsType(TYPE_XYZ) and c:IsAbleToRemove()
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 then
		...
		e7:SetCountLimit(1)
		e7:SetCondition(cm.resetcon)
		e7:SetOperation(cm.resetop)
		e7:SetLabel(rct)
...
function cm.mfilter(c)
	return c:IsSetCard(0xde5) and c:IsType(TYPE_MONSTER)
end
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and cm.mfilter(chkc) end
	if chk==0 then return true end
	local tc=e:GetLabelObject()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
	local g=Duel.SelectTarget(tp,cm.mfilter,tp,LOCATION_GRAVE,0,1,1,nil)
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if mc and tc:GetFlagEffect(m)~=0 and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)~=0 then
