...
function c115848157.regop(e,tp,eg,ep,ev,re,r,rp)
    ...
    e1:SetCost(c115848157.spcost)
    e1:SetTarget(c115848157.sptg)
    e1:SetOperation(c115848157.spop)
    e1:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
    c:RegisterEffect(e1)
end
function c115848157.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost() end
    Duel.Remove(e:GetHandler(),POS_FACEUP,REASON_COST)
end
function c115848157.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingMatchingCard(c115848157.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
