...
function c44447100.initial_effect(c)
	...
	e22:SetDescription(aux.Stringid(44447100,0))
	...
	e22:SetCountLimit(1)
	e22:SetCondition(c44447100.eqcon)
	e22:SetTarget(c44447100.eqtg)
	e22:SetOperation(c44447100.eqop)
...
--Equip
function c44447100.eqcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentPhase()==PHASE_MAIN1 or Duel.GetCurrentPhase()==PHASE_MAIN2
end
function c44447100.eqfilter(c)
	return not c:IsForbidden() and c:IsSetCard(0x644)
end
function c44447100.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 
		and Duel.IsExistingMatchingCard(c44447100.eqfilter,tp,LOCATION_GRAVE+LOCATION_DECK,0,1,nil,tp) end
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,nil,1,tp,LOCATION_GRAVE+LOCATION_DECK)
end
function c44447100.eqop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	if c:IsFacedown() or not c:IsRelateToEffect(e) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local tc=Duel.SelectMatchingCard(tp,c44447100.eqfilter,tp,LOCATION_GRAVE+LOCATION_DECK,0,1,1,nil,tp):GetFirst()
	if tc then
		...
		e1:SetValue(c44447100.eqlimit)
		tc:RegisterEffect(e1)
	end
end
function c44447100.eqlimit(e,c)
	return e:GetOwner()==c
end