...
function c44460061.initial_effect(c)
	--synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0x677),aux.NonTuner(nil),1)
	c:EnableReviveLimit()
	--sy
...
function c44460061.setfilter(c)
	return (c:IsSetCard(0x677) or c:IsSetCard(0x679)) and not c:IsForbidden() and c:IsType(TYPE_MONSTER) 
end
...
function c44460061.sfilter(c)
	return (c:IsSetCard(0x677) or c:IsSetCard(0x679)) and bit.band(c:GetOriginalType(),TYPE_MONSTER)~=0 
end
