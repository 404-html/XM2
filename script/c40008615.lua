...
function c40008615.initial_effect(c)
	...
	e2:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x6b1))
	e2:SetValue(500)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
...
function c40008615.atkfilter1(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSummonType(SUMMON_TYPE_XYZ) and c:IsControler(tp) and c:IsCanBeEffectTarget(e) and c:IsSetCard(0x6b1)
end
...
function c40008615.atkfilter(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSummonType(SUMMON_TYPE_XYZ) and c:IsControler(tp) and c:IsCanBeEffectTarget(e) and not c:IsSetCard(0x6b1)
end
...
function c40008615.thfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x6b1) and c:IsAbleToHand()
end
